...
## 2026-09-26
- [開発部] 個人順位の担当チーム名(.sub)が長い場合に折り返す不具合を修正。font-size .78→.66rem＋white-space:nowrap（.nameもnowrap）。「クソ雑魚 とかじ ファイトクラブ・ドリブンズ」がスマホ幅でも1行に収まることを確認
- [開発部] LINE配信スケジュールを土10:00→土06:00に前倒し（notify.yml cron: "0 21 * 9-12,1-3 5"＝UTC金21:00、README表も更新）。GitHub Actionsのcronはbest-effortで遅延しうる旨をコメント追記
